...
#define k_ipc_max_message_size  512
#define k_ipc_mutex_name        L"Local\\cartographer_ipc_mutex"
#define k_ipc_pipe_name         L"\\\\.\\pipe\\cartographer_ipc"
#define k_ipc_connect_timeout   5000
#define k_ipc_send_retry_count  10
#define k_ipc_send_retry_delay  100
...
static bool shell_windows_ipc_try_claim_server_role()
{
    ...
    switch (wait_result)
    {
	    ...
	    case WAIT_ABANDONED:
	        LOG_DEBUG_GAME("[{}]{}", __FUNCTION__, "Mutex was abandoned, reclaiming server role");
	        return true;
...
static void shell_windows_ipc_process_client(HANDLE pipe)
{
    ...
    {
        LOG_DEBUG_GAME("[{}]{}", __FUNCTION__, "Failed to read message from client");
        return;
    }

    if (bytes_read == 0)
    {
        LOG_DEBUG_GAME("[{}]{}", __FUNCTION__, "Failed to read data from client");
        return;
    }

    LOG_DEBUG_GAME("[{}]{}", __FUNCTION__, "Read message from client");
...
static DWORD WINAPI shell_windows_ipc_server_thread(LPVOID param)
{
    ...
    while (!g_ipc_state.shutdown_requested)
    {
        ...
        {
            LOG_DEBUG_GAME("[{}]{}", __FUNCTION__, "Processing client connection");
...
static void shell_windows_ipc_handle_join_message(uint8* session_buffer, uint32 buffer_size)
{
    ...
    LOG_DEBUG_GAME("[{}]{} {:x} {}", __FUNCTION__, "Proccessing join:", session.hostAddress.inaOnline.S_un.S_addr, session.hostAddress.wPortOnline);

    EnterCriticalSection(&g_ipc_message_lock);
    g_auto_join_session = session;
...
static void shell_windows_ipc_handle_message(uint8* data_buffer, uint32 buffer_size)
{
    ...
    LOG_DEBUG_GAME("[{}]{}: {}", __FUNCTION__, "Processing message of type", (uint8)type);
...
bool shell_windows_ipc_notify_session_info(XSESSION_INFO* session)
{
	...
	memcpy(&msg.session, session, sizeof(msg.session));
