	...
	if (weapon_index != NONE)
	{
		...
		if (weapon_data->animation_manager.valid())
		{
				...
				if (ratio > k_real_epsilon)
				{
					...
					if (weapon_data->jitter_animation_channel.valid())
					{
						weapon_data->jitter_animation_channel.apply_weighted_node_orientations(0.f,
							(weapon->weapon.overcharged + 0.5f) * ratio,
							0.f,
							weapon_data->node_orientations_count,
							fp_orientations->weapon_orientations,
							NULL,
				...
				)
			{
				...
				{
					int16 rounds_loaded_maximum = weapon->weapon.magazines[0].rounds_inventory;
					...
					{
						ammunition_frame_position = weapon->weapon.magazines[0].rounds_inventory;
					}
					else
					{
				...
				else
				{
					...
					position = (real32)PIN(weapon->weapon.magazines[0].rounds_inventory, 0, frame_count - 1);
				}
