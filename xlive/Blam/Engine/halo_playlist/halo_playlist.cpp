...
void c_halo_playlist_reader::process_match_section()
{
    ...
    if (!wcscmp(match->map, L""))
    {
        ...
        this->error(_halo_playlist_error_match_property_invalid, match->match_line_in_file, map_string);
    ...
    if (!wcscmp(match->variant, L""))
    {
        ...
        this->error(_halo_playlist_error_match_property_invalid, match->match_line_in_file, variant_string);
...
void c_halo_playlist_reader::process_variant_property(s_halo_playlist_section_item* section)
{
    ...
    {
        if (!wcscmp(section->name_buffer, this->m_section_items[i].name_buffer))
        {
...
bool c_halo_playlist_reader::property_name_is_valid(wchar_t* property_name)
{
    ...
    if (halo_playlist_item_collection_match_settings_property_get_value(property_name) != k_halo_playlist_match_settings_property_invalid)
        return true;
	...
	if (halo_playlist_item_collection_cartographer_setting_get_value(property_name) != k_halo_playlist_cartographer_setting_count)
		return true;
...
void c_halo_playlist_reader::error(e_halo_playlist_error error_type, uint32 file_line, wchar_t* property_name, wchar_t* property_value, wchar_t* extra)
{
    ...
    wcsncpy_s(error->property_name, NUMBEROF(error->property_name), property_name, _TRUNCATE);
    wcsncpy_s(error->property_value, NUMBEROF(error->property_value), property_value, _TRUNCATE);
    wcsncpy_s(error->property_extra, NUMBEROF(error->property_extra), extra, _TRUNCATE);
...
void c_halo_playlist_reader::finalize()
{
    ...
    for (uint32 i = 0; i < this->m_match_count; ++i)
    {
        ...
        if (!variant)
        {
            this->error(_halo_playlist_error_match_variant_not_found, match->variant_line_in_file);
            continue;
        }
        ...
        if (!map_found)
        {
            this->error(_halo_playlist_error_match_map_not_found, match->map_line_in_file);
            continue;
        }
...
int c_halo_playlist_reader::playlist_error_sort(void* context, void const* lhs, void const* rhs)
{
    ...
    int32 result = _wcsicmp(left->property_name, right->property_name);
    
	if (result)
        return result;

    result = _wcsicmp(left->property_value, right->property_value);

    if (result)
        return result;

    return _wcsicmp(left->property_extra, right->property_extra);
}
